...
func (c *ReservationController) Post() {
	...
	if form != "check" && form != "reserve" && form != "package" {
		c.Get()
	}
	...
	if form == "package" {
		...
		c.Data["Adults"], _ = c.GetInt("adults")
		c.Data["Childs"], _ = c.GetInt("childs")
		c.Data["Package"] = c.GetString("package")
		c.Data["SuccessMsg"] = "Packages are available on the requested date, you many now proceed with the booking."
		return
	}
